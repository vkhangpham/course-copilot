...
def test_syllabus_designer_uses_taxonomy_modules() -> None:
    designer = SyllabusDesigner()
    modules = designer.propose_modules(DATASET_ROOT)

    assert modules, "Expected modules derived from taxonomy.yaml"
    assert modules[0].title == "Relational Thinking & Data Modeling"
    assert "relational abstractions" in modules[0].outcomes[0].lower()
