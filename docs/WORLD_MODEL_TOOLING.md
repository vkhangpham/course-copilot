...
- **Config file:** `config/scientific_config.yaml` controls the evaluator module. Set `enabled: false` to turn it off globally, or flip individual toggles under `evaluation_metrics` (e.g., `blooms_taxonomy: false`). When you need a different profile per run/CI job, either export `COURSEGEN_SCIENCE_CONFIG=/abs/path/to/custom.yaml` or pass `--science-config /abs/path/to/custom.yaml` to `coursegen-poc`/`python apps/orchestrator/run_poc.py`. The run manifest now records this under `science_config_path`, and the CLI’s `[artifacts]` hint mirrors it as `science_config=/...` so auditors can trace every run to the exact config file.
