...
3. **Inspect data** – `wm-inspect concepts --store world_model/state.sqlite --topic transaction` (plus `timeline`, `claims`, `papers`, `authors`, `definitions`, `graph`, and `artifacts`) renders quick JSON tables without opening SQLite. Useful for debugging prompts and CodeAct tools. Pass `--type quiz_bank` or `--type course_outline` to the `artifacts` command to narrow the results when you only care about specific asset classes.
